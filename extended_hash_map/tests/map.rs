...
fn ploc_conditions_for_test_data() 
    -> (
        Vec<&'static str>, 
        Vec<HashMap<&'static str, i32>>
) {
    (
        vec![
            ">=1",
...
fn hash_map_iter() -> std::vec::IntoIter<(&'static str, i32)> {
    let mut hash_map = HashMap::from(TEST_DATA)
        .into_iter()
        .collect::<Vec<_>>();
...
fn create() {
    assert_eq!(
        ExtendedHashMap::<&str, i32>::new().is_empty(), 
        true
    );

    assert_eq!(
        ExtendedHashMap::<&str, i32>::default().is_empty(), 
        true
    );
}
...
fn ploc() {
    ...
    for (condition, hash_map) in conditions
        .iter()
        .zip(hash_maps.iter()) 
    {
...
fn iter() {
    ...
    for ((lhs_key, lhs_val), (rhs_key, rhs_val)) in map
        .iter()
        .zip(hash_map_iter()) 
    {
